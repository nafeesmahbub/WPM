...
function Timer(event){
    ...
    if(timer == 0 && event.which!=13 && stopped == 0){
      ...
      timer = 1;
      emit = true;
      setInterval(function(){ showStat(); }, 1000);
    }
...
function StopTime(){
    ...
    $('#WPM').replaceWith("<h2>Your typing speed in net WPM: " + netWPM + " words per minute." + "Errors: " + numErrors + " Time:" + seconds/10);
    $('#WPM').css('color', 'green');
    seconds = 0;
    timer = 0;
    emit = false;
...
function showStat(){
    if(emit){
        var netWPM = calcNetWPM();
        $("#wpm_"+sessionID).html(netWPM);
        //console.log("wpm_"+sessionID);
        socket.emit('show_WPM', {sessionID: sessionID, netWPM: netWPM});
        //sortTable();
        $("#wpm_header").trigger("click");
        console.log(stopped);
    }
}

function highlight(text) {
  inputText = document.getElementById("paragraph");
